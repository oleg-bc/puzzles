...
function isCryptSolution(crypt, solution) {
    ...
    for (let i = 0; i < solution.length; i++) {
        if (solution.length < 2 && crypt[i].length < 2 && solution[0][1] === "0") return true;
        if (crypt.length < 2) return true;
        if (solution[i][1] === '0' && trackArr.includes(solution[i][0])) {
            console.log("FALSE - STARTS WITH 0");
            return false;
        }
    }
    //return true;
    ...
    if (arrToSum[0] + arrToSum[1] !== arrToSum[2]) {
        console.log("DOESNT ADD UP AT THE END: FALSE", arrToSum[0], '+', arrToSum[1], '!=', arrToSum[2]);
        return false;
    }
    return true;
}
